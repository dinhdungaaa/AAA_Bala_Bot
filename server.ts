...
Hãy trình bày bố cục thông tin đẹp mắt, rõ ràng, dễ đọc, ngắt dòng khoa học, chuẩn phong cách nhắn tin trên Telegram.`;
  ...
  if (!isGoodMatch) {
    ...
    if (bot.tone === "friendly") {
      const opening = shouldGreet ? `Dạ em chào ${pronoun} ${targetName} ạ! ` : "";
      smartFallbackText = `${opening}Hiện tại thông tin chi tiết về câu hỏi "${query}" chưa có sẵn hoàn chỉnh trong dữ liệu tri thức của em rồi nha. Em đã ghi nhận câu hỏi này để gửi cho ban quản trị tiến hành cập nhật thêm vào tri thức hệ thống cho em sớm nhất ạ.

${pronoun === "chị" ? "Chị" : pronoun === "anh" ? "Anh" : "Anh/chị"} cứ yên tâm nhé! Lúc này, nếu cần phản hồi hỗ trợ khẩn cấp ngay, ${pronoun} ${targetName} liên lạc trực tiếp hotline SĐT ${bot.fallbackPhone} hoặc qua Zalo ${bot.fallbackZalo} giúp em nha.`;
    } else {
      const opening = shouldGreet ? `Kính gửi ${pronoun} ${targetName}, hiện` : "Hiện";
      smartFallbackText = `${opening} tại thông tin về câu hỏi "${query}" chưa có sẵn đầy đủ trong danh mục đào tạo của hệ thống. Chúng tôi đã ghi nhận nội dung câu hỏi để báo cáo ban quản trị tiến hành cập nhật thêm thông tin vào tri thức hệ thống sớm nhất.
