...
function postProcessBotReply(text: string) {
  return text
    .replace(/[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}\u{FE0F}\u{200D}]/gu, "")
    .replace(/\s+([,.!?;:])/g, "$1")
    .replace(/[ \t]{2,}/g, " ")
...
Hãy trình bày bố cục thông tin đẹp mắt, rõ ràng, dễ đọc, ngắt dòng khoa học, chuẩn phong cách nhắn tin trên Telegram.`;
      ...
      return {
        text: postProcessBotReply(responseText),
...
Để nhận thông tin hỗ trợ chính xác lập tiếp, kính mời ${pronoun} ${targetName} liên hệ trực tiếp qua Hotline ${bot.fallbackPhone} hoặc kết nối tài khoản Zalo ${bot.fallbackZalo} để chuyên viên chăm sóc ngay ạ.`;
    ...
    return {
      text: postProcessBotReply(smartFallbackText),
      sources: [],
  ...
  let replyText = "";
  if (bot.tone === "friendly") {
    replyText = `Dạ ${pronoun} ${targetName} ơi, về vấn đề "${query}" em xin gửi ${pronoun} ${targetName} thông tin từ tri thức của hệ thống nha:\n\n${primeChunk.title}: ${primeChunk.content}\n\nHi vọng thông tin này giúp ích được cho mình ạ.`;
  } else {
    replyText = `Kính gửi ${pronoun} ${targetName}, liên quan đến thông tin tìm kiếm: "${query}". Hệ thống xin phản hồi chính xác dựa trên danh mục huấn luyện:\n\n${primeChunk.title}: ${primeChunk.content}\n\nĐể biết thêm chi tiết, vui lòng liên hệ tổng đài ${bot.fallbackPhone}.`;
  }

  return {
    text: postProcessBotReply(replyText),
