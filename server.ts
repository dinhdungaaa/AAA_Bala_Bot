...
function sanitizeTelegramHTML(text: string): string {
  ...
  escaped = escaped
    ...
    .replace(/<i>/g, "___I_OPEN___")
    .replace(/<\/i>/g, "___I_CLOSE___");
  ...
  const openB = (escaped.match(/<b>/g) || []).length;
  const closeB = (escaped.match(/<\/b>/g) || []).length;
  if (openB > closeB) {
    escaped += "</b>".repeat(openB - closeB);
  }
  
  const openI = (escaped.match(/<i>/g) || []).length;
  const closeI = (escaped.match(/<\/i>/g) || []).length;
  if (openI > closeI) {
    escaped += "</i>".repeat(openI - closeI);
  }
...
async function generateRAGAnswer(
  ...
  let activeChunks: { chunk: KnowledgeChunk; score: number }[] = [];
  let isGoodMatch = false;

  // If the total chunks is small (<= 25), bypass matching and feed everything to Gemini for perfect recall
  if (botChunks.length <= 25 && botChunks.length > 0) {
    activeChunks = botChunks.map(chunk => ({ chunk, score: 1.0 }));
    isGoodMatch = true;
  } else {
    // 2. Multi-accent fuzzy phrase match search to rank chunks
    const queryLower = query.toLowerCase();
    const queryNormalized = removeVietnameseTones(query);
    const queryNormalizedWords = queryNormalized.split(/[\s,\.\?\!\-\_]+/);
    ...
    const matchedChunks = botChunks.map(chunk => {
      ...
      const contentNormalized = removeVietnameseTones(chunk.content);
      const titleNormalized = removeVietnameseTones(chunk.title);

      const contentNormalizedWords = contentNormalized.split(/[\s,\.\?\!\-\_]+/);
      const titleNormalizedWords = titleNormalized.split(/[\s,\.\?\!\-\_]+/);

      const contentWords = contentLower.split(/[\s,\.\?\!\-\_]+/);
      const titleWords = titleLower.split(/[\s,\.\?\!\-\_]+/);

      // Overlap scoring
      queryNormalizedWords.forEach(word => {
        if (word.length < 2) return;
        
        // Match in content
        if (contentNormalizedWords.includes(word)) {
          score += 0.08;
          // Accented match bonus
          const idx = contentNormalizedWords.indexOf(word);
          if (idx !== -1 && contentWords[idx] && contentWords[idx] === word) {
            score += 0.02;
          }
        }

        // Match in title
        if (titleNormalizedWords.includes(word)) {
          score += 0.24;
          // Accented match bonus
          const idx = titleNormalizedWords.indexOf(word);
          if (idx !== -1 && titleWords[idx] && titleWords[idx] === word) {
            score += 0.06;
          }
        }
      });
      ...
      // Substring normalized matches (vietnamese no tones)
      if (contentNormalized.includes(queryNormalized)) score += 0.6;
      if (titleNormalized.includes(queryNormalized)) score += 0.8;
    ...
    activeChunks = matchedChunks.slice(0, 10); // Retrieve top 10 matched chunks
    isGoodMatch = activeChunks.length > 0;
  }
  ...
  if (ai) {
    try {
      ...
      const contextString = activeChunks.length > 0
        ? activeChunks.map((item, id) => `[Thông tin nội bộ #${id + 1} - ${item.chunk.title}]:\n${item.chunk.content}`).join("\n\n")
        : "Hiện chưa có thông tin nội bộ cụ thể nào khớp với câu hỏi này.";

      // Natural pronoun and greeting customization
      let welcomePronoun = pronoun;
      let greetName = targetName;
      if (greetName === "Khách Hàng") {
        welcomePronoun = pronoun === "Anh/Chị" ? "quý khách" : pronoun;
        greetName = "";
      }
...
CẢNH BÁO NGHIÊM NGẶT VỀ CÁCH XỬ LÝ THÔNG TIN (VÔ CÙNG QUAN TRỌNG - VI PHẠM LÀ LỖI NGHIÊM TRỌNG):
- TUYỆT ĐỐI KHÔNG BAO GIỜ sao chép, dán nguyên văn, hoặc trích dẫn trực tiếp nội dung từ thông tin nội bộ (reference) vào câu trả lời. Bạn PHẢI đọc hiểu, tiêu hóa, rồi DIỄN ĐẠT LẠI bằng ngôn ngữ tự nhiên của chính mình như một chuyên viên tư vấn thực thụ đang trò chuyện.
- TUYỆT ĐỐI KHÔNG BAO GIỜ để lộ rằng mình đang đọc từ tài liệu, tri thức, dữ liệu nguồn, hoặc bất kỳ hệ thống nào. Cấm tuyệt đối các cụm từ: "theo tài liệu", "theo tri thức", "em xin gửi thông tin từ tri thức", "dựa trên tài liệu nguồn", "trong dữ liệu của em", "hệ thống tri thức", v.v. Hãy nói như thể bạn TỰ BIẾT thông tin đó vì bạn là nhân viên lâu năm của thương hiệu.
- KHÔNG BAO GIỜ gửi nguyên một đoạn dài liền mạch chứa toàn bộ nội dung tài liệu. Thay vào đó, hãy CHỌN LỌC những thông tin THỰC SỰ LIÊN QUAN đến câu hỏi cụ thể của khách, rồi diễn đạt ngắn gọn, súc tích, dễ hiểu.
- Hãy trả lời như một người tư vấn viên giàu kinh nghiệm đang ngồi trò chuyện trực tiếp: tự tin, am hiểu, thân thiện, KHÔNG BAO GIỜ lộ ra rằng mình đang tra cứu hay đọc tài liệu.

ĐỊNH DẠNG VĂN BẢN (BẮT BUỘC):
...
Dạ em chào quý khách ạ! Rất vui được đồng hành cùng mình ngày hôm nay nha.

Hiện tại bên em đang có chương trình đào tạo ứng dụng công nghệ AI vô cùng hiệu quả:

🔹 Giúp mình biết cách dùng AI để tối ưu hóa thời gian làm việc hàng ngày.

🔹 Hướng dẫn xây dựng một hệ thống tạo thu nhập bền vững.

Dạ không biết thông tin trên đã rõ ràng chưa hay mình cần em hỗ trợ giải đáp thêm phần nào khác nữa không ạ?

Ngôn ngữ trả lời bắt buộc: ${bot.language === 'vi' ? 'Tiếng Việt' : 'English'}.

QUY TẮC XỬ LÝ THÔNG TIN & FALLBACK:
1. Bạn THAM KHẢO thông tin nội bộ bên dưới để hỗ trợ tư vấn. Nhưng NHỚ: phải DIỄN ĐẠT LẠI, KHÔNG COPY-PASTE.
2. Nếu câu hỏi KHÔNG có trong thông tin nội bộ, bạn KHÔNG được bịa ra thông tin.
Thay vào đó, bạn BẮT BUỘC phải trả về từ khóa [FALLBACK] ở ngay đầu câu trả lời, rồi trả lời một cách KHÔN KHÉO, TỰ NHIÊN. Cách trả lời fallback đúng:
  - KHÔNG nói "chưa có trong tài liệu/tri thức/dữ liệu của em" (vì sẽ lộ bot).
  - Thay vào đó, hãy nói theo kiểu nhân viên thật: "Dạ, câu hỏi của mình rất hay ạ! Để em ghi nhận lại và xác nhận thông tin chính xác nhất cho mình nha" hoặc "Dạ phần này em cần kiểm tra lại với bên phòng ban liên quan để trả lời chính xác nhất cho mình ạ".
  - Sau đó khuyên khách liên hệ hotline hoặc Zalo để được tư vấn kĩ hơn.
Ví dụ đúng:
[FALLBACK] Dạ câu hỏi của mình rất hay ạ! Để em ghi nhận và xác nhận lại thông tin chi tiết nhất cho mình nha. Mình có thể liên hệ trực tiếp qua hotline hoặc Zalo để được tư vấn nhanh hơn ạ!
3. Bán hàng & Báo giá: ${bot.allowPricing ? 'CHO PHÉP cung cấp đơn giá, chính sách khuyến mãi nếu em biết.' : 'Tuyệt đối KHÔNG ĐƯỢC báo giá lẻ, khéo léo nói rằng giá sản phẩm có thể thay đổi tùy chương trình và hướng dẫn khách liên hệ hotline/Zalo để được báo giá chính xác nhất.'}
4. Tư vấn kỹ thuật sản phẩm: ${bot.allowProductConsulting ? 'CHO PHÉP giải thích chi tiết, cặn kẽ về sản phẩm.' : 'Chỉ giới thiệu tổng quan, không đi quá sâu vào các thông số kỹ thuật phức tạp.'}
5. Các chủ đề bị cấm trả lời tuyệt đối: "${bot.restrictedTopics}". Nếu khách vi phạm hoặc hỏi lạc đề này, hãy khéo léo hướng họ về sản phẩm và dịch vụ cốt lõi của thương hiệu.

THÔNG TIN NỘI BỘ (CHỈ ĐỌC ĐỂ THAM KHẢO - TUYỆT ĐỐI KHÔNG COPY-PASTE NGUYÊN VĂN):
${contextString}
...
Hãy trình bày bố cục thông tin đẹp mắt, rõ ràng, dễ đọc, ngắt dòng khoa học, chuẩn phong cách nhắn tin trên Telegram.`;
      ...
      // Detect LLM-Driven Fallback
      let isFallback = responseText.includes("[FALLBACK]") || 
                       !isGoodMatch ||
                       responseText.toLowerCase().includes("em chưa có thông tin") || 
                       responseText.toLowerCase().includes("kiểm tra lại với") ||
                       responseText.toLowerCase().includes("xác nhận lại thông tin") ||
                       responseText.toLowerCase().includes("ghi nhận lại");

      // Strip [FALLBACK] tag from final response text if present
      if (responseText.includes("[FALLBACK]")) {
        isFallback = true;
        responseText = responseText.replace("[FALLBACK]", "").trim();
      }
      ...
      return {
        text: responseText,
        sources: isGoodMatch ? activeChunks.map(m => ({ id: m.chunk.id, name: m.chunk.title, score: Math.min(0.99, 0.4 + (m.score === 1.0 ? 0.59 : m.score)) })) : [],
  ...
  if (!isGoodMatch) {
    ...
    if (bot.tone === "friendly") {
      smartFallbackText = `Dạ em chào ${fullGreet} ạ! Hiện tại thông tin chi tiết về câu hỏi "${query}" chưa có sẵn hoàn chỉnh trong dữ liệu tri thức của em rồi nha. Em đã ghi nhận câu hỏi này để gửi cho ban quản trị tiến hành cập nhật thêm vào tri thức hệ thống cho em sớm nhất ạ.

${pronoun === "chị" ? "Chị" : pronoun === "anh" ? "Anh" : "Anh/Chị"} cứ yên tâm nhé! Lúc này, nếu cần phản hồi hỗ trợ khẩn cấp ngay, ${fullGreet} liên lạc trực tiếp hotline SĐT ${bot.fallbackPhone} hoặc qua Zalo ${bot.fallbackZalo} giúp em nha! ❤️`;
    } else {
      smartFallbackText = `Kính gửi ${fullGreet}, hiện tại thông tin về câu hỏi "${query}" chưa có sẵn đầy đủ trong danh mục đào tạo của hệ thống. Chúng tôi đã ghi nhận nội dung câu hỏi để báo cáo ban quản trị tiến hành cập nhật thêm thông tin vào tri thức hệ thống sớm nhất.

Để nhận thông tin hỗ trợ chính xác lập tiếp, kính mời ${fullGreet} liên hệ trực tiếp qua Hotline ${bot.fallbackPhone} hoặc kết nối tài khoản Zalo ${bot.fallbackZalo} để chuyên viên chăm sóc ngay ạ.`;
    }
  ...
  let replyText = "";
  if (bot.tone === "friendly") {
    replyText = `Dạ ${fullGreet} ơi, về vấn đề "${query}" em xin gửi ${fullGreet} thông tin từ tri thức của AAA Farm nha:\n\n👉 *${primeChunk.title}*: ${primeChunk.content}\n\nHi vọng sẽ giúp ích được cho mình ạ! 💚`;
  } else {
    replyText = `Kính gửi ${fullGreet}, liên quan đến thông tin tìm kiếm: "${query}". AAA Farm xin phản hồi chính xác dựa trên danh mục huấn luyện:\n\n📖 *${primeChunk.title}*: ${primeChunk.content}\n\nĐể biết thêm chi tiết, vui lòng liên kết tổng đài ${bot.fallbackPhone}.`;
  }
