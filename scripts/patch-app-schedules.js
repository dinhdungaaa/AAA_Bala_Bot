...
// ============================================================
// 1. Add schedule state variables after kbSearchQuery
// ============================================================
const stateMarker = `// Search Knowledge Filter
  const [kbSearchQuery, setKbSearchQuery] = useState('');`;
const stateReplacement = `// Search Knowledge Filter
  const [kbSearchQuery, setKbSearchQuery] = useState('');
...
if (content.includes(stateMarker)) {
  ...
  console.log("[1] Added schedule state variables");
} else {
  console.log("[1] SKIP - state marker not found");
}

// ============================================================
// 2. Add sidebar nav button for schedules (after Billing button)
// ============================================================
const billingBtnEnd = `<CreditCard className="w-4 h-4 text-rose-400" />
            Gói Cước & Bảng Giá
          </button>`;
const schedSidebarBtn = `<CreditCard className="w-4 h-4 text-rose-400" />
            Gói Cước & Bảng Giá
          </button>
...
if (content.includes(billingBtnEnd)) {
  content = content.replace(billingBtnEnd, schedSidebarBtn);
  console.log("[2] Added sidebar button for schedules");
} else {
  console.log("[2] SKIP - billing button end not found");
}

// ============================================================
// 3. Add header title for schedules tab
// ============================================================
const analyticsHeader = `{activeTab === 'analytics' && 'Báo cáo Đo Lường Hiệu Suất'}`;
const schedHeader = `{activeTab === 'analytics' && 'Báo cáo Đo Lường Hiệu Suất'}
                  {activeTab === 'schedules' && 'Hệ Thống Nhắc Lịch Tự Động & AI Push'}`;

if (content.includes(analyticsHeader)) {
  content = content.replace(analyticsHeader, schedHeader);
  console.log("[3] Added header title for schedules");
} else {
  console.log("[3] SKIP - analytics header not found, trying variant...");
  // Try with encoded characters
  const alt = "activeTab === 'analytics'";
  const lines = content.split('\n');
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes(alt) && lines[i].includes('Hiệu Suất')) {
      lines.splice(i + 1, 0, `                  {activeTab === 'schedules' && 'Hệ Thống Nhắc Lịch Tự Động & AI Push'}`);
      content = lines.join('\n');
      console.log("[3] Added header title (alt method) at line", i + 1);
      break;
    }
  }
}
...
// ============================================================  
// 4. Add fetchSchedules in useEffect for bot resources
// ============================================================
const fetchFaqsLine = `fetch(\`/api/bots/\${selectedBotId}/faqs\`)
      .then(res => res.json())
      .then(data => setFaqs(data));`;
const fetchFaqsPlusSchedules = `fetch(\`/api/bots/\${selectedBotId}/faqs\`)
      .then(res => res.json())
      .then(data => setFaqs(data));
...
if (content.includes(fetchFaqsLine)) {
  content = content.replace(fetchFaqsLine, fetchFaqsPlusSchedules);
  console.log("[4] Added schedule fetching in useEffect");
} else {
  console.log("[4] SKIP - fetchFaqs not found");
}

// ============================================================
// 5. Add schedule tab content BEFORE admin tab
// ============================================================
const adminTabStart = `{activeTab === 'admin' && sbUser?.email === ADMIN_EMAIL && (`;
const scheduleTabContent = `{activeTab === 'schedules' && (
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-1 justify-end">
                                  <button onClick={async () => {
                                    await fetch(\`/api/schedules/\${sched.id}/toggle\`, { method: 'PUT' });
                                    const res = await fetch(\`/api/bots/\${selectedBotId}/schedules\`);
                                    setSchedules(await res.json());
                                  }} className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer" title={sched.status === 'active' ? 'Tạm dừng' : 'Bật lại'}>
                                    <Power className={\`w-3.5 h-3.5 \${sched.status === 'active' ? 'text-green-500' : 'text-slate-400'}\`} />
                                  </button>
                                  <button onClick={async () => {
                                    if (!confirm('Gửi nhắc nhở ngay lập tức?')) return;
                                    await fetch(\`/api/schedules/\${sched.id}/trigger-now\`, { method: 'POST' });
                                    const res = await fetch(\`/api/bots/\${selectedBotId}/schedules\`);
                                    setSchedules(await res.json());
                                    const lr = await fetch(\`/api/bots/\${selectedBotId}/reminder-logs\`);
                                    setRemLogs(await lr.json());
                                    alert('Đã gửi nhắc nhở thành công!');
                                  }} className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer" title="Gửi ngay">
                                  ...
                                  }} className="p-1.5 hover:bg-rose-50 rounded-lg transition-colors cursor-pointer" title="Xóa">
              ...
              {schedTab === 'create' && (
                <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs space-y-5">
                  <h3 className="font-bold text-base text-slate-800 flex items-center gap-2">
                    ...
                    Tạo Lịch Nhắc Mới (Chat Tay Trên Web)
                  </h3>
                    ...
                    try {
                      ...
                      const res = await fetch(\`/api/bots/\${selectedBotId}/schedules\`, {
                        ...
                        body: JSON.stringify({
                          ...
                          maxTriggers: schedForm.maxTriggers > 0 ? schedForm.maxTriggers : undefined
                        })
                      });
                      if (res.ok) {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <select value={schedForm.frequency} onChange={e => setSchedForm({ ...schedForm, frequency: e.target.value })}
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 cursor-pointer">
                          <option value="once">Một lần</option>
                          <option value="daily">Hàng ngày</option>
                          <option value="weekdays">T2 - T6</option>
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Giới hạn lần nhắc (0 = vô hạn)</label>
                    ...
                    <div className="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-xl p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-bold text-slate-800">AI Push Nhân Viên</span>
                        </div>
                      ...
                      {schedForm.aiEnhanced && (
                        <div>
                          ...
                          <p className="text-[10px] text-purple-500 mt-2">Gemini AI sẽ viết lại nội dung mỗi lần gửi theo tone đã chọn, không bao giờ lặp lại.</p>
                        </div>
                      )}
                    </div>

                    <button type="submit" disabled={schedLoading}
                      className="w-full py-3 bg-teal-600 hover:bg-teal-700 disabled:bg-slate-300 text-white font-bold text-sm rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md shadow-teal-500/10">
                ...
                <div className="space-y-5">
                  ...
                  <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs space-y-4">
                    <h3 className="font-bold text-base text-slate-800 flex items-center gap-2">
                      ...
                      Nạp File Quy Trình (TXT, CSV, JSON, Excel)
                    </h3>
                    <div className="border-2 border-dashed border-slate-300 hover:border-teal-400 rounded-xl p-8 text-center transition-colors cursor-pointer"
                      ...
                      <input id="sched-file-input" type="file" accept=".txt,.csv,.json,.xlsx,.xls,.md,.text" className="hidden"
                        onChange={e => setSchedUploadFile(e.target.files?.[0] || null)} />
                    </div>
                    <button disabled={!schedUploadFile || schedLoading} onClick={async () => {
                      ...
                      try {
                        ...
                        reader.onload = async () => {
                          ...
                          const res = await fetch(\`/api/bots/\${selectedBotId}/schedules/upload\`, {
                            method: 'POST', headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ fileName: schedUploadFile.name, fileData: base64 })
                          });
                          const data = await res.json();
                          if (data.success) {
                            setSchedules(prev => [...data.schedules, ...prev]);
                            alert(\`Nạp thành công \${data.totalParsed} lịch nhắc từ file!\`);
                            setSchedUploadFile(null);
                            setSchedTab('list');
                          } else {
                            alert('Lỗi: ' + (data.errors?.join(', ') || 'Không thể parse file'));
                          }
                          setSchedLoading(false);
                        };
                        reader.readAsDataURL(schedUploadFile);
                      } catch (err: any) { alert('Lỗi: ' + err.message); setSchedLoading(false); }
                  ...
                  <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs space-y-4">
                    <h3 className="font-bold text-base text-slate-800 flex items-center gap-2">
                      ...
                      AI Parse Văn Bản Tự Do
                    </h3>
                    <p className="text-xs text-slate-500">Nhập mô tả quy trình bằng ngôn ngữ tự nhiên. AI sẽ tự động trích xuất thành danh sách lịch nhắc có cấu trúc.</p>
                    <textarea rows={5} placeholder={"VD: Nhắc họp sáng lúc 8h30 mỗi ngày.\\nBáo cáo doanh thu vào 17h chiều thứ 6 hàng tuần.\\nNhắc kiểm kê kho vào 9h ngày 1 mỗi tháng."} value={schedParseText}
                      onChange={e => setSchedParseText(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 resize-none" />
                    <button disabled={!schedParseText.trim() || schedLoading} onClick={async () => {
                      if (!selectedBotId) return;
                      setSchedLoading(true);
                      try {
                        const res = await fetch(\`/api/bots/\${selectedBotId}/schedules/parse-text\`, {
                          method: 'POST', headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({ text: schedParseText })
                        });
                        const data = await res.json();
                        if (data.success) {
                          setSchedules(prev => [...data.schedules, ...prev]);
                          alert(\`AI đã trích xuất thành công \${data.totalParsed} lịch nhắc!\`);
                          setSchedParseText('');
                          setSchedTab('list');
                        } else {
                          alert('Lỗi: ' + (data.errors?.join(', ') || 'AI không thể phân tích'));
                        }
                      } catch (err: any) { alert('Lỗi: ' + err.message); }
                      finally { setSchedLoading(false); }
              ...
              {schedTab === 'logs' && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={async () => {
                      const res = await fetch(\`/api/bots/\${selectedBotId}/reminder-logs\`);
                      setRemLogs(await res.json());
                    }} className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-bold rounded-lg transition-colors cursor-pointer flex items-center gap-1">
                  ...
                  {remLogs.length === 0 ? (
                    <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
                      ...
                      <p className="text-sm text-slate-500">Chưa có nhắc nhở nào được gửi. Lịch sử sẽ xuất hiện khi scheduler engine bắt đầu trigger.</p>
                    </div>
                  ) : (
                      ...
                      {remLogs.map(log => (
                        <div key={log.id} className={\`bg-white rounded-xl border p-4 \${log.status === 'sent' ? 'border-green-200' : log.status === 'failed' ? 'border-rose-200' : 'border-slate-200'}\`}>
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <span className={\`text-[10px] px-2 py-0.5 rounded-full font-bold border \${log.status === 'sent' ? 'bg-green-50 text-green-600 border-green-200' : log.status === 'failed' ? 'bg-rose-50 text-rose-600 border-rose-200' : 'bg-slate-100 text-slate-500 border-slate-200'}\`}>
                          ...
                          {log.errorMessage && <p className="text-[10px] text-rose-500 mt-1">Lỗi: {log.errorMessage}</p>}
                        </div>
                      ))}
                    </div>
                  )}
...
if (content.includes(adminTabStart)) {
  content = content.replace(adminTabStart, scheduleTabContent);
  console.log("[5] Added schedule tab content before admin tab");
} else {
  console.log("[5] SKIP - admin tab start not found");
}
