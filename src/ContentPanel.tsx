...
const POST_TYPES: Record<PostType, PostTypeInfo> = {
  ...
  D3: { name: 'Hot take / opinion (ngược dòng)', group: 'Thương hiệu cá nhân', words: '200–400 từ',
        formula: 'Hook ngược dòng → Lập luận 2–3 lý do → Bằng chứng cá nhân → Kết + câu hỏi mở' },
  D7: { name: 'Behind-the-scenes', group: 'Thương hiệu cá nhân', words: '200–400 từ',
        formula: 'Hé lộ hậu trường/quá trình thật → thử–sai → con người thật → CTA gần gũi' },
  D2: { name: 'Chia sẻ insight / kiến thức', group: 'Quảng bá / kiến thức', words: '300–500 từ',
        formula: 'Hook sự thật bất ngờ → Vì sao quan trọng → Framework 3–5 điểm → Ví dụ thật → CTA save/share' },
  D4: { name: 'How-to / tutorial', group: 'Quảng bá / kiến thức', words: '400–700 từ',
...
export function ContentPanel({ botId }: { botId: string | null | undefined }) {
  ...
  const loadVoice = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) { const d = await res.json(); setVoice(d.voice || ''); }
    } catch { /* im lặng */ }
  }, [botId]);
  ...
  }, [botId, loadUsage, loadPosts, loadVoice]);
